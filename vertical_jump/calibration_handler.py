...
import csv
import os
from .handlers import PoseHandler

class CalibrationHandler():
    def __init__(self, source_name="", jumper_name="Ben", jumper_height=72, jump_style=0, vid_format=0, log=logging.getLogger(__name__)):
        '''
    ...
    def measure_descent_speed(self):
        ...
        self.log.info(f"measure_descent_speed(self) |Init Frame = {init_frame}")
        self.log.info(f"measure_descent_speed(self) |Init Level = {init_level}")
        self.log.info(f"measure_descent_speed(self) |Descent Frame = {descent_frame}")
        self.log.info(f"measure_descent_speed(self) |Descent Level = {lowest_point}\n")
        self.log.info(f"measure_descent_speed(self) |Final Ground Frame = {final_ground_frame}")
        ...
        self.log.info(f"measure_descent_speed(self) |Lowest Reached (inches): {descent_level:.2f}")
        self.log.info(f"measure_descent_speed(self) |Descent Speed (inches/second): {descent_speed:.2f}")
        self.log.info(f"measure_descent_speed(self) |Ground Time: {ground_time:.2f} seconds")
    ...
    def play_selected_video(self, draw_pose=False):
        ...
        while True:
            ...
            if draw_pose:
                ...
                frame = self.convert_to_formatted_frame(frame)
